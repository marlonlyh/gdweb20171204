...
//	$.ajax({
//		type: "get",
//		url: '/static/js/queryAllCities.js',
//		async: false,
//		dataType: "json",
...
function sendAllProvinceAjax() {
	$.ajax({
		type: "get",
		url: '/static/js/queryAllProvinces.js',
		async: false,
		dataType: "json",
		success: function(data) {
...
function sendAllCountiesAjax()
 {
	$.ajax({
		type: "get",
		url: "/static/js/queryAllAreas.js",
		async: false,
		dataType: "json",
		success: function(data) {
			allAreas = data.areas;
...
function viewAllCities(i) {
	...
	var province = allProvinces[i].provinceName
	alert(province);
	allCitys = [];
					...
					success: function(result) {
						...
						if (result.code!="0"){
							...
							$.each(result.data,
								...
								var li = $('<li><a href="javascript:onclick=viewAllCounties(' + data.CID + ')" id="' + data.CID + '">' + data.CNAME + '</a></li>');
	...
	$.ajax({
		...
		success: function(result) {
			...
			if (result.code!="0"){
				...
				$.each(result.data,
						function(i, data) {
					$("body").data("cAllId", data.LEVEL_CODE);
					countiesAll=data;
					$("body").data("nameOfCityAll", data.NAME);
					var li = $('<li><a href="javascript:onclick=addrInputAll(' + i + ')" id="' + data.LEVEL_CODE+ '">' +  data.NAME + '</a></li>');
