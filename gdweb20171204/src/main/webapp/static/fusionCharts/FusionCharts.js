...
infosoftglobal.FusionCharts.prototype = {
	...
	setDataURL: function(strDataURL){
		if (this.initialDataSet==false){
			...
			if (!chartObj.setDataURL)__flash__addCallback(chartObj, "setDataURL");
...
//a:str,b:drawID,c:viewType,d:width,e:height,f:flashID
function drawChart(a,b){
	//swf, id, w, h, debugMode, registerWithJS, c, scaleMode, lang, detectFlashVersion, autoInstallRedirect
	var DATA=a;
	if(typeof(a)=='string'){
		var t=a.split("|");
		if(String(t[0])=='0'){
		...
		else if(String(t[0])=='quit'){
			alert("提示[EC01024]：\n\n你可能未登录，请重新登录！");
			return top.location.replace("../Include/Exit.asp?act=exit");
		}
		else{
			DATA=t[0]
		}
	}
	var p={w:352,h:288,debug:0,js:0,bgcolor:'FFFFFF',scale:0,id:"AutoName_"+Math.ceil(Math.random()*1000)},d="/Public/Charts/";for(var k in b)p[k]=b[k];
	p.scale=p.scale!=0?'exactFit':'noScale';

	p.swf=escape(p.swf);

//alert(p.swf+'\n'+id+'\n'+p.w+'\n'+p.h+'\n'+p.debug+'\n'+p.rwjs+'\n'+p.bgcolor+'\n'+p.scale)
//show('<pre>SWF:'+p.swf+'\nID:'+id+'\nWidth:'+p.w+'\nHeight:'+p.h+'\nDebug:'+p.debug+'\nBgColor:'+p.bgcolor+'\nScaleMode:'+p.scale+'\n\nDATA:\n'+a.replaceAll("<",'&lt;').replaceAll('>','&gt;')+'</pre>','ADIV',{mode:'append'});
	var chartLog = new FusionCharts(p.swf,p.id,p.w,p.h,p.debug,p.js,p.bgcolor,p.scale);
	chartLog.setDataXML(DATA);
	chartLog.render(p.id);
}