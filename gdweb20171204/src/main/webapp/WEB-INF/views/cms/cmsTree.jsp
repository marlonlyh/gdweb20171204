	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			for(var i=0; i<zNodes.length; i++) {
				...
				//移除企业中心
				if (zNodes[i] && zNodes[i].id == "${fns:getDictValue('front.enterpriseCenter','comm_category_id','2014')}"){
					zNodes.splice(i, 1);
				}
				//移除原材料报价
				if (zNodes[i] && zNodes[i].id == "${fns:getDictValue('front.source','comm_category_id','2001')}"){
					zNodes.splice(i, 1);
				}
				//移除产品中心
				if (zNodes[i] && zNodes[i].id == "${fns:getDictValue('front.product','comm_category_id','2025')}"){
					zNodes.splice(i, 1);
				}
				//移除论坛
				if (zNodes[i] && zNodes[i].id == "${fns:getDictValue('front.forum','comm_category_id','2013')}"){
					zNodes.splice(i, 1);
				}
				<%--
				// 并将没有关联关系的父节点，改为父节点
