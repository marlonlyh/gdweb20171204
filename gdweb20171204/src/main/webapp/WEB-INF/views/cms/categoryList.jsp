...
<body>
	...
	<form id="listForm" method="post">
		<table id="treeTable" class="table table-striped table-bordered table-condensed">
			<tr><th>栏目名称</th><th>栏目简称</th><th>栏目模型</th><th style="text-align:center;">排序</th>
			<th title="是否在导航中显示该栏目">导航菜单</th><th title="是否在分类页中显示该栏目的文章列表">栏目列表</th><th title="是否在app导航中显示该栏目">app栏目</th><th>展现方式</th><th>app展示列表</th><th>app展示内容</th><th>操作</th></tr>
			<c:forEach items="${list}" var="tpl">
				<tr id="${tpl.id}" pId="${tpl.parent.id ne '1' ? tpl.parent.id : '0'}">
					...
					<td>${fns:getDictLabel(tpl.appinMenu, 'show_hide', '隐藏')}</td>
					<td>${fns:getDictLabel(tpl.showModes, 'cms_show_modes', '默认展现方式')}</td>
					<td>${fns:getDictLabel(tpl.showflag, 'cms_showflag', '文字展示列表')}</td>
					<td>${fns:getDictLabel(tpl.showcontent, 'cms_showcontent', '图文')}</td>
					<td>
