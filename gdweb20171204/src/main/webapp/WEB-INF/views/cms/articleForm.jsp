...
<body>
	...
	<form:form id="inputForm" modelAttribute="article" action="${ctx}/cms/article/save" method="post" class="form-horizontal">
		...
		<%--  <div class="control-group">
			...
			<div class="controls">
				<form:select path="year" class="input-mini">
					<form:option value="" label="默认"/>
					<form:options items="${fns:getDictList('company_year')}" itemLabel="label" itemValue="value" htmlEscape="false" />
				</form:select>
			</div>
		</div> --%>
		...
		<c:if test="${descriptionHidden ne 1}">
			<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="articleData.author" htmlEscape="false" maxlength="200" class="input-xlarge"/>
			</div>
		</div>
		<!--  
		<div class="control-group">
		...
		<%-- <div class="control-group">
			...
			<div class="controls">
				<input id="finishDate" name="finishDate" type="text" maxlength="20" class="input-medium Wdate"
					value="<fmt:formatDate value="${article.finishDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
					onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
			</div>
		</div> --%>
