...
//	        page.setCount(Long.valueOf(createQuery(countQlString, parameter).uniqueResult().toString()));
	...
	public void delete(Serializable id){
		...
		setParameter(query, new Parameter(id));
		query.executeUpdate();
	}
...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
	...
	public Page<T> find(Page<T> page, DetachedCriteria detachedCriteria, ResultTransformer resultTransformer) {
		...
		if (!page.isDisabled() && !page.isNotCount()){
			...
			long lcount=page.getCount();
			int lPageNo=page.getPageNo();
			int lpageSize=page.getPageSize();
			
			int lfirst=1;
			int llast=(int)(lcount / (lpageSize < 1 ? 20 : lpageSize) + lfirst - 1);
			if (lcount % lpageSize != 0 || llast == 0) {
				llast=llast+1;
			}	
			int lTotalPage=llast;
			
			if ((lcount < 1) || (lPageNo>lTotalPage) || (lPageNo<=0)) {
				//System.out.println(ReflectionToStringBuilder.toString(page));
				return page;
			}
				
//			if ((page.getCount() < 1) || (page.getPageNo()>page.getTotalPage()) || (page.getPageNo()<=0)) {
//				//System.out.println(ReflectionToStringBuilder.toString(page));
//				return page;
//			}
		}
