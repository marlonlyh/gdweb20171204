...
public class Category extends IdEntity<Category> {
	...
	private String shortEn; //简称
	private String image; 	// 栏目图片
	private String banner;//栏目广告图片
   ...
   private String appinMenu; 		// app是否在导航中显示（1：显示；0：不显示）
   private String showflag; 	// app展示列表（0:图片展示列表;1：文字展示列表；2：单页; 3：招聘展示; 4：班级风采展示;）
   private String showcontent; 	// app展示内容（0:图文;1：多图；2：视频）
	...
	@Length(min=1, max=1)
	public String getAppinMenu() {
		return appinMenu;
	}

	public void setAppinMenu(String appinMenu) {
		this.appinMenu = appinMenu;
	}
    ...
    @Transient
    @JsonIgnore
    public List<Category> getValidList() {
    	List<Category> vlist = Lists.newArrayList();
    	
    	for (Category cat: this.childList) {
    		if (StringUtils.equals(cat.getInMenu(),Category.SHOW) && StringUtils.equals(cat.getDelFlag(), Category.DEL_FLAG_NORMAL)) {
    			vlist.add(cat);
    		}
    	}
