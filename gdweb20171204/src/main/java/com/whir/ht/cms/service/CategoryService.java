...
//			dc.add(Restrictions.or(Restrictions.isNull("href"),Restrictions.eq("href", "")));
	...
	public List<Category> findByCategorys(String appinMenu){
		return categoryDao.findByCategorys(appinMenu);
	}
	
	public List<Category> findByParentId(String parentId, String siteId,String appinMenu){
		return categoryDao.findByParentIdAndSiteIdAndApp(parentId, siteId,appinMenu);
	}
	...
	public Page<Category> find(Page<Category> page, Category category) {
		...
		if (StringUtils.isNotBlank(category.getAppinMenu()) && Category.SHOW.equals(category.getAppinMenu())){
			dc.add(Restrictions.eq("appinMenu", category.getAppinMenu()));
		}
...
//		return page;
	...
	public Page<Category> findPages (Page<Category> page, Category category) {
		...
		dc.createAlias("parent", "parent");
		dc.add(Restrictions.eq("parent.id", category.getId()));	
	 ...
	 * @return
	 */
	public List<Category> findSiteModule(String module,String site){
		DetachedCriteria dc = categoryDao.createDetachedCriteria();	
		dc.add(Restrictions.like("module", "%"+module+"%"));
		dc.add(Restrictions.eq("site.id", site));
		dc.add(Restrictions.eq(Category.FIELD_DEL_FLAG, Category.DEL_FLAG_NORMAL));
